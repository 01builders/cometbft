...
func (env *Environment) SignedBlock(ctx *rpctypes.Context, heightPtr *int64) (*ctypes.ResultSignedBlock, error) {
	...
	block, _ := env.BlockStore.LoadBlock(height)
	if block == nil {
...
func (env *Environment) DataCommitment(ctx *rpctypes.Context, start, end uint64) (*ctypes.ResultDataCommitment, error) {
	...
	root, err := env.hashDataRootTuples(tuples)
	if err != nil {
		return nil, err
	}
...
func (env *Environment) To32PaddedHexBytes(number uint64) ([]byte, error) {
...
func (env *Environment) EncodeDataRootTuple(height uint64, dataRoot [32]byte) ([]byte, error) {
	paddedHeight, err := env.To32PaddedHexBytes(height)
	if err != nil {
		return nil, err
	}
...
func (env *Environment) hashDataRootTuples(tuples []DataRootTuple) ([]byte, error) {
	dataRootEncodedTuples := make([][]byte, 0, len(tuples))
	for _, tuple := range tuples {
		encodedTuple, err := env.EncodeDataRootTuple(
			tuple.height,
			tuple.dataRoot,
		)
		if err != nil {
