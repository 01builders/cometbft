...
import (
	...
	tmproto "github.com/cometbft/cometbft/api/cometbft/types" // need to add proto changes
...
func (rp RowProof) Validate(root []byte) error {
	...
	if int(rp.EndRow-rp.StartRow1) != len(rp.RowRoots) {
		return fmt.Errorf("the number of rows %d must equal the number of row roots %d", int(rp.EndRow-rp.StartRow1), len(rp.RowRoots))
	}
