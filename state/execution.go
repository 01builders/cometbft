...
func (blockExec *BlockExecutor) ApplyVerifiedBlock(
	state State, blockID types.BlockID, block *types.Block, syncingToHeight int64, lastCommit *types.Commit,
) (State, error) {
	return blockExec.applyBlock(state, blockID, block, syncingToHeight, lastCommit)
}
...
func (blockExec *BlockExecutor) ApplyBlock(
	state State, blockID types.BlockID, block *types.Block, syncingToHeight int64, commit *types.Commit,
) (State, error) {
	if err := validateBlock(state, block); err != nil {
		return state, ErrInvalidBlock(err)
	}

	return blockExec.applyBlock(state, blockID, block, syncingToHeight, commit)
}

func (blockExec *BlockExecutor) applyBlock(state State, blockID types.BlockID, block *types.Block, syncingToHeight int64, commit *types.Commit) (State, error) {
	...
	fireEvents(blockExec.logger, blockExec.eventBus, block, blockID, abciResponse, validatorUpdates, state.Validators, commit)
